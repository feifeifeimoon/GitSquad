...
- **`issue-detail`** — **one scroll container** for the whole page, holding a two-cell grid (`lg:grid-cols-[minmax(0,1fr)_15rem]`); the details column is `sticky` inside that scroll rather than a second scroller. Three independent scroll regions used to mean the facts could scroll away from the issue they described. Reading order is title → description → activity → composer, and the composer is last because that is where the newest entry belongs.
...
- **`activity-rail`** — one tick per entry down the right edge of the feed, from `RAIL_MIN_ENTRIES` (4) upward; below that it is furniture. A **roving focus**: one tab stop, with arrows and Home/End moving between ticks, so twenty comments are not twenty stops between the feed and the composer. `IntersectionObserver` marks the entries in the band, so the rail answers "where am I" as well as "take me there". Kind is not encoded in the dash — at 2 px, agent versus person read as a rendering artifact — so colour is spent on position instead: in view is `{colors.ink}`, the rest `{colors.hairline-strong}`. Each 2 px dash sits inside a 24×24 button, because a rail you cannot hit is not a rail.
- **`issue-actions`** — `Copy link` / `Copy {ISSUE_KEY}` written out as rows with their icons, not hidden behind an icon-only menu; the aside has room and the labels are the only thing that makes them findable.

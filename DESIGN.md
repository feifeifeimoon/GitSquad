...
- **`issue-heading`** — the title as an `<h1>` with a pencil button beside it; `↵` saves, `⎋` abandons. The heading is **not** the control. A button whose accessible name is its own text makes the title a name for every other button on the page — a locator for "Comment" then resolves to an issue called "Comment target", and the click lands on the heading. The description follows the same rule, putting its pencil under the prose it edits.
...
- **`activity-rail`** — one tick per entry down the right edge of the feed, from `RAIL_MIN_ENTRIES` (4) upward; below that it is furniture. It is **the height of the scroll viewport, never of the thread** — it sticks inside the feed and is clamped to `viewport − 64px`, so the ticks compress as a thread grows instead of drawing a dashed line down the whole page. Each tick's position is **measured from the entry it stands for** (`getBoundingClientRect` against the feed's own extent), not laid out in flow: a tick you cannot line up with anything is decoration. A **roving focus**: one tab stop, with arrows and Home/End moving between ticks, so twenty comments are not twenty stops between the feed and the composer. The entries inside the reading band are drawn at full strength, so the rail answers "where am I" as well as "take me there". Kind is not encoded in the dash — at 2 px, agent versus person read as a rendering artifact — so colour is spent on position instead: in view is `{colors.ink}`, the rest `{colors.hairline-strong}`. Each 2 px dash sits inside a 20×20 button, because a rail you cannot hit is not a rail.
- **`issue-actions`** — `Copy link` and `Copy {ISSUE_KEY}` as **icon buttons in the page's top-right corner**, which is where Linear keeps them and what the eye already reads as "actions on this thing". They were a labelled list in the details rail, where they sat under the facts and read as two more facts. Icon-only means the label has to survive as the tooltip and the accessible name, and the copy is acknowledged in place by swapping the icon for a check rather than by a toast.
